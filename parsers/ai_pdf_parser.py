...
주어진 텍스트와 이미지에서 모든 구조 부재의 설계 정보를 추출하여 반드시 순수 JSON만 반환하세요.
마크다운 코드블록(```)을 절대 사용하지 말고, JSON 텍스트만 출력하세요.

=== 이미지 분석 지시 ===
페이지 이미지가 함께 제공됩니다. 이미지에서 다음을 반드시 확인하세요:
1. 단면도(Section Drawing)의 치수: 콘크리트 표면~철근 중심 거리가 Loc 값입니다.
   예: 단면도에 "0.04"라고 적혀있으면 Loc = 40mm
2. 단면도에서 상부/하부 Loc가 다를 수 있습니다. 각각 Loc_top_mm, Loc_bot_mm에 저장.
3. T형보인지 직사각형 보인지 단면도 형상으로 판별하세요.
4. 텍스트에서 추출 못한 값도 이미지에서 읽을 수 있으면 반드시 포함하세요.
...
3. (-) 부 모멘트와 (+) 정 모멘트를 구분하여 추출. 모든 값은 절대값(양수)으로 저장
4. 하중조합 번호는 배열로 저장. 예: [11, 25, 15]
5. ★★★ 분리 규칙 ★★★
   같은 부재명(예: "PBT")이 MIDAS Gen과 BeST.RC/BeST.Steel 양쪽에서 나오면
   반드시 별도 객체로 분리하라. 절대 합치지 마라.
   - MIDAS Gen 결과와 BeST 결과는 배근, 피복두께, 설계강도가 다를 수 있다.
   - software 필드로 구분: "MIDAS Gen" 또는 "BeST.RC" 등
   - 결과적으로 같은 이름이 2개 존재할 수 있으며, software 필드로 구분한다.
6. 텍스트에 없는 값은 null로 표시
7. 기둥 단면: MIDAS Gen은 단면 치수가 텍스트에 없을 수 있음 → BeST에서 가져오기
8. 배근 정보: "3-D19", "2-D10@125" 등을 문자열로 저장
10. Loc 값: BeST.RC에서 "Loc. = 35 mm" 또는 "Loc. = 40 mm" 형태로 나오면
    Loc_top_mm, Loc_bot_mm에 저장. 콘크리트 표면에서 철근 중심까지 거리(mm).
    MIDAS Gen 단면도의 치수(0.04 등)도 Loc일 수 있으나 확실하지 않으면 null
...
def extract_relevant_pages(pdf_path: str) -> tuple:
    """pdfplumber로 관련 페이지 텍스트 추출 + PyMuPDF로 이미지 추출"""
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for i, page in enumerate(pdf.pages):
            ...
            if any(kw in text for kw in RELEVANT_KW):
                pages.append({"page": i + 1, "text": text, "page_idx": i})

    # PyMuPDF로 관련 페이지 이미지 추출
    if PYMUPDF_OK and pages:
        try:
            doc = fitz.open(pdf_path)
            for p in pages:
                fitz_page = doc[p["page_idx"]]
                pix = fitz_page.get_pixmap(dpi=150)
                img_bytes = pix.tobytes("png")
                p["image_base64"] = base64.b64encode(img_bytes).decode()
            doc.close()
        except Exception:
            # 이미지 추출 실패해도 텍스트는 사용 가능
            pass
...
def parse_pdf_with_ai(pdf_path: str, api_key: str = None) -> dict:
    ...
    # 2. Claude API 호출 — 텍스트 + 이미지
    client = anthropic.Anthropic(api_key=api_key)

    # 메시지 content 구성: 이미지가 있으면 텍스트+이미지 멀티모달
    user_content = []

    # 각 페이지의 이미지 추가
    has_images = any(p.get("image_base64") for p in pages)
    if has_images:
        for p in pages:
            if p.get("image_base64"):
                ...
                user_content.append({
                    "type": "text",
                    "text": f"↑ 페이지 {p['page']} 이미지",
                })

    # 텍스트 추가
    user_content.append({
        "type": "text",
        "text": f"다음 구조계산서에서 모든 부재 정보를 추출하세요.\n"
                f"이미지에서 단면도의 Loc 치수, 배근 상세도 확인하세요.\n\n"
                f"=== 추출된 텍스트 ===\n{combined}",
    })
    ...
    message = client.messages.create(
        ...
        messages=[{
            "role": "user",
            "content": user_content if has_images else f"다음 구조계산서 텍스트에서 모든 부재 정보를 추출하세요:\n\n{combined}",
        }],
    )
    ...
    _img_count = sum(1 for p in pages if p.get("image_base64"))
    return {
        "members":      data.get("members", []),
        "pages_total":  total,
        "pages_used":   len(pages),
        "images_sent":  _img_count,
        "raw_text":     combined,
